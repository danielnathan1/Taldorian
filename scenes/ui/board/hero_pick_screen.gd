# scenes/ui/board/hero_pick_screen.gd
extends Control
...
func _rebuild_heroes() -> void:
	...
	for hero in GameState.players[local_idx].heroes:
		...
		# Expande para preencher o espaço disponível no HeroesRow
		slot.custom_minimum_size = Vector2(0, 0)
		slot.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		slot.size_flags_vertical   = Control.SIZE_EXPAND_FILL
		slot.bind(hero)
		# apenas heróis ativos são clicáveis
...
func _refresh_views() -> void:
	for slot in _slots:
		if not is_instance_valid(slot):
			continue
		slot.refresh()  # restaura modulate de estado (active/exhausted/defeated)
